...
def _build_args(x_in__, indice_out_, values_out_, dimension, keep_dims):
    __inputs__ = []
    for arg in [x_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [indice_out_, values_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if keep_dims != None:
        ...
        attr["name"] = "keep_dims"
        attr["dtype"] = "bool"
        attr["value"] = keep_dims
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("ArgMaxWithValue", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def arg_max_with_value(x_in__, indice_out_, values_out_, dimension, keep_dims=False, kernel_name="arg_max_with_value", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, indice_out_, values_out_, dimension, keep_dims)
    ...
    origin_func_name = "arg_max_with_value"
    ascendc_src_dir_ex = "arg_max_with_value"
    ascendc_src_dir = "arg_max_with_value"
    ascendc_src_file = "arg_max_with_value.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator ArgMaxWithValue. kernel name is " + kernel_name)
    op_type = "ArgMaxWithValue"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__], origin_outputs = [indice_out_, values_out_],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required'], init_value_list = [None, None],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(x_in__, indice_out_, values_out_, dimension, keep_dims=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, indice_out_, values_out_, dimension, keep_dims)
    result = check_op_cap("op_select_format", "ArgMaxWithValue", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, indice_out_, values_out_, dimension, keep_dims=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, indice_out_, values_out_, dimension, keep_dims)
    result = check_op_cap("get_op_specific_info", "ArgMaxWithValue", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
